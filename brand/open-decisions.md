# Decisiones abiertas de marca y marketing

Estado: `pending_review`

## Identidad

- Aprobar misión, visión y manifiesto públicos.
- Definir si “Granja Paraíso” se comunica públicamente o permanece como visión interna.
- Elegir eslogan o decidir operar inicialmente sin uno.
- Aprobar, ajustar o reemplazar el logo provisional de lanzamiento y luego ampliar paleta,
  tipografías y sistema visual.
- Definir el protagonismo visual de la luna, las aves, la naturaleza y la tecnología.

## Comercial

- Definir el catálogo inicial y el mecanismo para consultar disponibilidad y precios.
- Establecer políticas de reserva, entrega, cancelación y atención posventa.
- Definir cuándo una línea o híbrido tiene evidencia suficiente para comunicar sus atributos.
- Decidir cuándo presentar públicamente las futuras pollonas Black Star.

## Contenido

- Autorizar el nivel de exposición de cada integrante de la familia.
- Acordar por separado cualquier participación de menores.
- Definir una frecuencia sostenible para Facebook antes de abrir Instagram.
- Decidir qué instalaciones pueden aparecer sin afectar seguridad o privacidad.
- Definir si la comunicación pública indicará sólo Paraguay o también Coronel Oviedo.
- Seleccionar métricas de los primeros tres meses.

## Canales

- Crear y configurar la página oficial de Facebook con el paquete preparado en
  `brand/facebook-launch-ready.md`.
- Reservar nombre de usuario coherente en Instagram.
- Definir WhatsApp Business, información pública y horario de atención.
- Evaluar YouTube, TikTok y sitio web después de validar el flujo inicial.
